              ...
              <Link to={`/productdetails/${product?.id}`} key={product?.id}>
                <div className='product'  title={product?.title}>
                    ...
                    <div className='product-price'>
                        <h4>₹{(Math.round((product?.price *50) - (Math.ceil(product?.discountPercentage)/100 * (product?.price *50)))).toLocaleString()}</h4>
                        <del className='prod-price'>₹{(product?.price * 50).toLocaleString()}</del>
                        <div className='product-off'>{Math.ceil(product?.discountPercentage)}%off</div>
                    </div>
                </div></Link>
            ))
        }
